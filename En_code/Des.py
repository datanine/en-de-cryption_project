...
class DESCipher:
    ...
    def PC1change(self, key):
        result = table_mapping(PC1, key)
        return result

    # PC2置换---
    def PC2change(self, endkey):
        result = table_mapping(PC2, endkey)
        return result
    ...
    def init_permutation(self, block_plain_text):
        result = table_mapping(IP, block_plain_text)
        return result
    ...
    def Echange(self, tmp):
        result = table_mapping(E, tmp)
        return result

    # 末(rIP)置换---
    def reinitpermutation(self, tmp):
        result = table_mapping(rIP, tmp)
        return result
